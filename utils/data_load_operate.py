...
def sampling_SVM(ratio_list, num_list, gt_reshape, class_count, Flag):
...
def sampling(ratio_list, num_list, gt_reshape, class_count, Flag):
    ...
    for cls in range(class_count):
        ...
        # print(len(test_label_index_dict[cls]))
    ...
    # return train_label_index_list, val_label_index_list, test_label_index_list # SVM
...
def HSI_create_pathes(data_padded, hsi_h, hsi_w, data_indexes, patch_length, flag):
    h_p, w_p, c = data_padded.shape # ([526, 628, 106])

    data_size = len(data_indexes) # h*w
    ...
    if flag == 1:
        ...
        unit_data = np.zeros((data_size, patch_size, patch_size, c),dtype=np.float32)
...
def generate_iter_1(data_padded, hsi_h, hsi_w, label_reshape, index, patch_length, batch_size, model_type_flag,
    ...
    if model_type_flag == 1:  # data for single spatial net
        if model_flag == 1:
            ...
            spa_val_samples = HSI_create_pathes(spa_dr_data_torch, hsi_h, hsi_w, index[1], patch_length, 1)
            spa_test_samples = HSI_create_pathes(spa_dr_data_torch, hsi_h, hsi_w, index[2], patch_length, 1)
        else:
    ...
    train_iter = Data.DataLoader(dataset=torch_dataset_train, batch_size=batch_size, shuffle=True, num_workers=0) # ,drop_last=True
...
def generate_iter_total(data_padded, hsi_h, hsi_w, label_reshape, index, patch_length, batch_size,
                        model_type_flag,
                        model_3D_spa_flag , model_flag, dr_c):
    ...
    if model_type_flag == 1:
        if model_flag == 1:
            total_samples = HSI_create_pathes(spa_dr_data_torch, hsi_h, hsi_w, index, patch_length, 1)
        else:
            total_samples = HSI_create_pathes(data_padded_torch, hsi_h, hsi_w, index, patch_length, 1)
        if model_3D_spa_flag == 1:  # spatial 3D patch
            total_samples = total_samples.unsqueeze(1)
